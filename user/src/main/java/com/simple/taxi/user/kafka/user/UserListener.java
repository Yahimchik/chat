...
public class UserListener {
    ...
    public void listen(@Header("kafka_receivedTopic") String topic, String message, Acknowledgment ack) {
        try {
            UserEvent event = objectMapper.readValue(message, UserEvent.class);
            dispatcher.dispatch(topic, event);
            ack.acknowledge(); // ✅ подтверждаем вручную
            log.info("✅ Processed event from topic {}", topic);
        } catch (Exception e) {
